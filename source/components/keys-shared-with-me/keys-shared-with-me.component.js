...
const KeysSharedWithMe = ({ list, navigation, onImport }) => (
    ...
    <View style={styles.titleIcon}>
          ...
          <TouchableOpacity
            ...
            onPress={() => navigate('KeySharedWithMe', navigation)}>
          </TouchableOpacity>
        </View>
        <FlatList
          data={list.sharedKeys}
          renderItem={({item}) => keyList(item, navigation, onImport)}
...
const keyList = (item, navigation, onImport) => {
  console.log("Shared: ", item)
  return(
      <TouchableRipple
        rippleColor="rgba(0, 0, 0, .32)"
        onPress={() => navigate('KeySharedWithMe', navigation, item)}>
        <View>
          <View style={styles.titleSubTitle}>
            <Title style={styles.catTitle}>{item.name}</Title>
            <Caption>{item.ownerName}</Caption>
            <View style={styles.arrowView}>
              {item.lockCode ? <TouchableOpacity>
                <Icon
                name="close"
                color="rgb(33, 47, 60)"
                size={15}
