...
import {View, SafeAreaView, ScrollView, FlatList, TouchableOpacity, Alert} from 'react-native';
...
componentDidMount() {
  ...
  cloneArray = (data) => {
    ...
    removeElement = (item, array, flag) => {
        ...
        flag === 'edit' ? this.setState({ editors: array }) : this.setState({ viewers: array })
    }
    ...
    [
      {text: 'DELETE', onPress: () => this.deleteKey(key)},
      {text: 'CANCEL'},
    ],
    {cancelable: false},
  );
    ...
    chip = (item, arr, flag) => (
        ...
        <Chip
              onPress={() => {this.removeElement(item, arr, flag)}}
              style={styles.tiny}
