...
import {getKeys, getSharedKeys, createNewKey, importKey} from '../../configuration/api/api.functions'
...
class KeyRing extends Component {
  ...
  addTheKey = async () => {
    ...
    if (addKeyName.length !== 0){
        ...
        .then(response => {
            this.setState({ addKeyName: '', addKeyModal: false }, () => this.getMyKeys())
        })
        .catch(error => {
  ...
  importKeyCode = async () => {
      ...
      .then(response => {
          ...
          this.setState({ importKey: '', addKeyModal: false }, () => this.getSharedKeys())
      })
      .catch(error => {
  ...
  modal = () => (
    <Modal visible={this.state.addKeyModal} transparent={true} animationType="fade">
      ...
      <SafeAreaView style={styles.outerView}>
          <ScrollView keyboardShouldPersistTaps="handled">
              ...
              <View style={styles.outerView}>
                <MyKeys keyList={myKeyList} navigation={navigation} onPress={() => this.setState({ addKeyModal: true, showImport: false })}/>
              </View>
              <View style={styles.outerView}>
                <KeysSharedWithMe list={sharedKeyList} navigation={navigation} onImport={(id) => this.setState({ addKeyModal: true, showImport: true, keyId: id })}/>
              </View>
              {this.modal()}
            </View>
          </ScrollView>
